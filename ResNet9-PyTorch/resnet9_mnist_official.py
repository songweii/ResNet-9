...
class ResNet9(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        self.conv4 = self._make_conv_block(256, 256, pool=True)
        self.res2 = nn.Sequential(self._make_conv_block(256, 256), self._make_conv_block(256, 256))
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.res1(x) + x  # torch.Size([8, 128, 14, 14])
        x = self.conv3(x)     # torch.Size([8, 256, 7, 7])
        x = self.conv4(x)     # torch.Size([8, 256, 3, 3])
        x = self.res2(x) + x  # torch.Size([8, 256, 3, 3])
        x = self.classifier(x)
        return x
...
def main():
    ...
    print(f"Official (w/ pytorch) Execution time: {total_time:.4f} seconds")
    dir = os.path.dirname(os.path.abspath(__file__))
    filepath = os.path.join(dir, 'official_training_history.pkl')
    picpath = os.path.join(dir, 'official_loss_accuracy_curve.png')
    # 保存batch损失和准确率数据到文件
