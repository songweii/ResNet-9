...
class fc_sigmoid:
    def __init__(self, in_channels, out_channels):
        ...
        self.init_param()
        self.m_kernel, self.v_kernel = cp.zeros_like(self.kernel), cp.zeros_like(self.kernel)
        self.m_bias, self.v_bias = cp.zeros_like(self.bias), cp.zeros_like(self.bias)
    ...
    def backward(self, out_diff_tensor, lr, beta1=0.9, beta2=0.999):
        ...
        # Adam: update paramters
        self.m_kernel += (1 - beta1) * (kernel_diff - self.m_kernel)
        self.v_kernel += (1 - beta2) * (kernel_diff**2 - self.v_kernel)

        self.m_bias += (1 - beta1) * (bias_diff - self.m_bias)
        self.v_bias += (1 - beta2) * (bias_diff**2 - self.v_bias)

        self.kernel -= lr * self.m_kernel / (cp.sqrt(self.v_kernel) + 1e-7)
        self.bias -= lr * self.m_bias / (cp.sqrt(self.v_bias) + 1e-7)
...
class conv_layer:
    def __init__(self, in_channels, out_channels, kernel_h, kernel_w, same=True, stride=1, shift=True):
        ...
        self.init_param()
        self.m_kernel, self.v_kernel = cp.zeros_like(self.kernel), cp.zeros_like(self.kernel)
        self.m_bias, self.v_bias = None, None
        if self.shift:
            self.m_bias, self.v_bias = cp.zeros_like(self.bias), cp.zeros_like(self.bias)
...
class max_pooling:
    ...
    def backward(self, out_diff_tensor, lr=0, beta1=0.9, beta2=0.999):
        assert out_diff_tensor.shape == self.out_tensor.shape
        batch_num = out_diff_tensor.shape[0]
...
class bn_layer:
    def __init__(self, neural_num, moving_rate=0.1):
        ...
        self.is_train = True
        self.epsilon = 1e-5
        self.m_kernel, self.v_kernel = cp.zeros_like(self.gamma), cp.zeros_like(self.gamma)
        self.m_bias, self.v_bias = cp.zeros_like(self.bias), cp.zeros_like(self.bias)
    ...
    def backward(self, out_diff_tensor, lr, beta1=0.9, beta2=0.999):
        ...
        # Adam: update paramters
        self.m_kernel += (1 - beta1) * (gamma_diff - self.m_kernel)
        self.v_kernel += (1 - beta2) * (gamma_diff**2 - self.v_kernel)

        self.m_bias += (1 - beta1) * (bias_diff - self.m_bias)
        self.v_bias += (1 - beta2) * (bias_diff**2 - self.v_bias)

        self.gamma -= lr * self.m_kernel / (cp.sqrt(self.v_kernel) + 1e-7)
        self.bias -= lr * self.m_bias / (cp.sqrt(self.v_bias) + 1e-7)
