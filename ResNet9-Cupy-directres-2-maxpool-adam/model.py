...
class ResBlock:
    ...
    def backward(self, out_diff_tensor, lr, beta1=0.9, beta2=0.999):
        ...
        self.relu.backward(out_diff_tensor, lr, beta1=beta1, beta2=beta2)
        x1 = self.relu.in_diff_tensor
        x2 = x1.copy()

        for l in range(1, len(self.path1) + 1):
            self.path1[-l].backward(x1, lr, beta1=beta1, beta2=beta2)
            x1 = self.path1[-l].in_diff_tensor

        if self.path2 is not None:
            for l in range(1, len(self.path2) + 1):
                self.path2[-l].backward(x2, lr, beta1=beta1, beta2=beta2)
...
class ResNet9:
    def __init__(self, num_classes):
        ...
        self.max_pool = max_pooling(2, 2, 2, same=False)
    ...
    def backward(self, out_diff_tensor, lr, beta1=0.9, beta2=0.999):
        x = out_diff_tensor
        self.fc.backward(x, lr, beta1=beta1, beta2=beta2)
        x = self.fc.in_diff_tensor
        self.max_pool.backward(x, lr, beta1=beta1, beta2=beta2)
        x = self.max_pool.in_diff_tensor

        for layer in range(1, len(self.layer3) + 1):
            self.layer3[-layer].backward(x, lr, beta1=beta1, beta2=beta2)
            x = self.layer3[-layer].in_diff_tensor
        for layer in range(1, len(self.layer2) + 1):
            self.layer2[-layer].backward(x, lr, beta1=beta1, beta2=beta2)
            x = self.layer2[-layer].in_diff_tensor
        for layer in range(1, len(self.layer1) + 1):
            self.layer1[-layer].backward(x, lr, beta1=beta1, beta2=beta2)
            x = self.layer1[-layer].in_diff_tensor
        for layer in range(1, len(self.pre) + 1):
            self.pre[-layer].backward(x, lr, beta1=beta1, beta2=beta2)
